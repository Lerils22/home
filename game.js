document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- GAME CONTROL ---
    async function startGame() { // NEW: Make the function async
        menu.classList.add('hidden');

        // NEW: Enter fullscreen and lock orientation on mobile
        const isMobile = window.innerWidth < 768; // Simple check for mobile
        if (isMobile) {
            const container = document.documentElement; // Target the whole page for fullscreen
            try {
                ...
                // 2. Lock to Landscape (may not work on all browsers, e.g., iOS)
                if (screen.orientation && screen.orientation.lock) {
                    await screen.orientation.lock('landscape');
                }
            } catch (err) {
                console.warn("Could not activate fullscreen or lock orientation:", err);
            }
        }
